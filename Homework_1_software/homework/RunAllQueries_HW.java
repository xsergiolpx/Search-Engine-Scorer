...
public class RunAllQueries_HW {
	...
	public static void main( String arg[] ) throws Exception {
		...
		if (arg.length != 5) {
			System.out.println("\nWrong number of arguments : given "+arg.length+" arguments, the correct number is 5.\\\n");
			System.out.println("\nCorrect use:\n");
			System.out.println(" java homework.RunAllQueries_HW \"collection_name\" file_with_all_queries_complete_path \"scorer_function\" \"ratio_between_title_and_text\" output_file_name");
			System.out.println();
			System.out.println(" Example of collection_name: \"cran\"");
			System.out.println(" Example of file_with_all_queries_complete_path: ./cran_all_queries.tsv");
			System.out.println(" scorer_function in {\"CountScorer\", \"TfIdfScorer\", \"BM25Scorer\"}");
			System.out.println(" fields in {\"text_and_title\", \"text\", \"title\"}");
			System.out.println(" Example of output file name: output_cran__NoStemmer__TfIdf.tsv");
			System.out.println();
			System.out.println("  Complete Example:");
			System.out.println("   java homework.RunAllQueries_HW \"cran\" ./cran_all_queries.tsv \"TfIdfScorer\" \"text_and_title\" output_cran__NoStemmer__TfIdf.tsv");
			System.out.println();
			return;
		}
		...
		if (!arg[3].equals("text_and_title") && !arg[3].equals("text") && !arg[3].equals("title")) {
			System.out.println();
			System.out.println(" field \"" + arg[3] + "\"" + " is not allowed.");
			System.out.println();
			System.out.println(" fields in {\"text_and_title\", \"text\", \"title\"}");
			System.out.println();
			return;
		...
		String all_queries_file = arg[1];
		BufferedReader br = new BufferedReader(
				   new InputStreamReader(
		                      new FileInputStream(all_queries_file), "UTF8"), 1000000);
		TreeMap<Integer, String> id_query = new TreeMap<Integer, String>();
		String all_content = "";
		String line = "";
		String[] line_toks = null;
		br.readLine();
		while ((line = br.readLine()) != null) {
			line_toks = line.split("\t");
			id_query.put(Integer.parseInt(line_toks[0]), line_toks[1]);
		}
		br.close();
		
		int text_weight = 1;
		int title_weight = 1;
		String index_to_query = arg[3];
		//boolean query_on_title = true;
		//boolean query_on_text  = true;
		//if (arg[3].equals("text")) {
		//	query_on_title = false;
		//} else if (arg[3].equals("title")) {
		//	query_on_text = false;
		//}
		...
		BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(arg[4]),"UTF-8"), 1000000);
		out.write("Query_ID"+"\t"+"Doc_ID"+"\t"+"Rank"+"\t"+"Score"+"\n");
		int external_doc_id = 0;
		int rank = 0;
		Integer c_query_id = 0;
		String c_query = "";
		String query = "";
		for (Map.Entry<Integer, String> entry : id_query.entrySet()) {
			c_query_id = entry.getKey();
			c_query = entry.getValue();
			query = RunAllQueries_HW.create_disjunctive_query(c_query, filter_stopwords, index_to_query);
		
			/* The query engine can return any subsegment of the results of a query. Here we grab the first 20 results. */
			engine.process( query, 0, 200, result );
			
			
			System.out.println();
			System.out.println( "Query_ID"+"\t"+"Doc_ID"+"\t"+"Rank"+"\t"+"Score");
			rank = 0;
			for( DocumentScoreInfo<Reference2ObjectMap<Index, SelectedInterval[]>> dsi : result ) {
				...
				rank++;
				external_doc_id = doc_id__file_id__mapping[(int)dsi.document];
				//System.out.println(external_doc_id + "\t" + dsi.document + "\t" + (""+dsi.score).replace('.', ',') );
				//System.out.println(c_query_id+"\t"+external_doc_id+"\t"+rank+"\t"+ ((""+dsi.score).replace('.', ',')));
				System.out.println(c_query_id+"\t"+external_doc_id+"\t"+rank+"\t"+ dsi.score);
				out.write(c_query_id+"\t"+external_doc_id+"\t"+rank+"\t"+dsi.score+"\n");
			}
		}
		out.close();
		...
		System.out.println("USED TERM PROCESSOR: " + termProcessors);
		System.out.println();
		/*
		for (Map.Entry<Integer, String> entry : id_query.entrySet()) {
	...
	protected static String create_disjunctive_query(String input_query, boolean filter_stopwords, String index_to_query) {
		String[] tok_query = input_query.split(" ");
		String query = "";
		...
		String output_query = "";
		if (index_to_query.equals("title")) {
			output_query = "title:" + "(" + query + ")";
		} else if (index_to_query.equals("text")) {
			output_query = "text:" + "(" + query + ")";
		} else {
			output_query = "title:" + "(" + query + ")" + " | " + "text:" + "(" + query + ")";
		}
